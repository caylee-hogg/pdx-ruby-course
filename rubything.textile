...
* Outside of the class, @@value@ can be as @c.value@ and @c.value = 5@
...
%add description of main page, ability to edit rants, add rants, etc.
%basic data schema should be two models: rants -> (id, message, userid)
%                                        user -> (id, name)
%authentication or no authentication?
* What's behind the curtain?
** Rails provides classes that allow us to retrieve and manipulate data easily 
** Logic for the application is written in normal Ruby, no explicit DB munging

h1. At The Console

* Close down the Rails server
* Run @rails console@
* Can modify the data using @Rant@ and @User@ classes
* @Rant@ class has @id@, @message@, and @user_id@ values.
...
Notice that you didn't need to save when you use @create@
...
just like with creation, we need to @save@ the changes.

h1. Where's My Stuff

%include demonstrations of find commands
...
%talk about validation, show how validation works with our application

h1. Displaying the data

%show the .erb files, explain how all that works

h1. Exercises

% Change the display to show messages with names instead of ids
...
* Run @rails betterrant@
* Run (insert actual scaffolding commands)
